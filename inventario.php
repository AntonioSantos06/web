...
if (isset($_POST['agregar'])) {
    $stmt = $conexion->prepare("INSERT INTO truper (clave, n_producto, marca, existencia, fisico) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssis", $_POST['clave'], $_POST['n_producto'], $_POST['marca'], $_POST['existencia'], $_POST['fisico']);
    $stmt->execute();
}

if (isset($_GET['eliminar'])) {
    $conexion->query("DELETE FROM truper WHERE clave='" . $conexion->real_escape_string($_GET['eliminar']) . "'");
}

if (isset($_POST['modificar'])) {
    $stmt = $conexion->prepare("UPDATE truper SET n_producto=?, marca=?, existencia=?, fisico=? WHERE clave=?");
...
<header>🔧 Gestión de Productos TRUPER<br><br>
<div class="nav">
